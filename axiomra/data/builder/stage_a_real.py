"""Stage A Real Provider Runner — 10 Real NSE Stocks x 6 Months."""
...
def run_stage_a_real_build(
    output_dir: str | Path = "axiomra-data/stage-a-real",
    raw_dir: str | Path = "axiomra-data/raw-stage-a-real",
    token: str | None = None,
) -> DatasetBuildResult:
    """Execute real Stage A dataset build across 10 liquid NSE stocks for 6 months."""
    ...
    config = DatasetBuildConfig(
        universe_name="STAGE-A-10-REAL",
        symbols=STAGE_A_REAL_SYMBOLS,
        start_date=start_dt,
        end_date=end_dt,
...
if __name__ == "__main__":
    res = run_stage_a_real_build()
    print(f"Stage A Real Build Complete! Dataset ID: {res.snapshot.dataset_id}")
